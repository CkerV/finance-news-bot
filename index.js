...
function saveLastNewsId(id) {
  writeFileSync(
    LAST_NEWS_FILE,
    JSON.stringify({ lastId: id, updatedAt: new Date().toISOString() }),
  );
}
...
async function sendToWechat(newsList) {
  ...
  const content = sortedNews
    .map((news, index) => {
      const newsTitle = news.title || news.highlight_title || "";
      const newsContent = news.content_text;
      const time = new Date(news.display_time * 1000).toLocaleString("zh-CN", {
...
async function main() {
  ...
  if (newNewsList.length > 0) {
    ...
    newNewsList.forEach((news, i) => {
      console.log(
        `  ${i + 1}. [${news.id}] ${news.content_text.substring(0, 30)}...`,
      );
    });
